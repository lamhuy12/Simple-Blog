...
public class RegisterServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(accountDAO.checkEmailIsExist(username)){
                request.setAttribute("REGISTER", "Fail");
            }
            else{
                boolean result = accountDAO.registerAccount(username, password, fullname, address);
                if(result){
                    request.setAttribute("REGISTER", "Success");
                }
            }
        }catch(NullPointerException ex){
            log("RegisterServlet _ NullPoint " + ex.getMessage());
